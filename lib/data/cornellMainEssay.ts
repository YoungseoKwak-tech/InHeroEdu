/**
 * Shared data for the Cornell Engineering Common App main essay ("죽은 해파리")
 * — the English paragraphs + paragraph-by-paragraph Korean analysis. Used by
 * /parents/essay and the 합격 프로필 (/parents/admits) Common App section.
 */

export interface Note { tag: string; text: string; weak?: string; }
export interface Segment { label: string; en: string; gist: string; notes: Note[]; }
