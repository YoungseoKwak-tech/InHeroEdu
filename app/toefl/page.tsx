...
const SECTIONS = [
  ...
  { tag: "Listening", title: "Listening · ~36 min", desc: "2 conversations + 3 lectures (28 questions total). Play the audio, listen, and answer in MCQ format — gist, detail, function, attitude, and organization types. Transcript revealed after grading.", color: "#0284c7" },
  { tag: "Speaking", title: "Speaking · 16 min", desc: "4 tasks (1 independent + 3 integrated). 15–30 sec prep → 45–60 sec response, with mic recording + speech-to-text transcription, then rubric-based self-scoring.", color: "#d97706" },
  { tag: "Writing", title: "Writing · ~29 min", desc: "Integrated (reading + listening summary) + Academic Discussion (opinion). Timed, with a live word count and rubric-based self-scoring.", color: "#7c3aed" },
];
...
export default function ToeflLanding() {
  ...
  return (
    <div style={{ background: "#ffffff", minHeight: "100vh", color: INK, fontFamily: "'Inter', -apple-system, sans-serif" }}>
      <PromoBanner ctaHref="/toefl/test?pay=1" sticky={false} navOffset={64} />
      ...
      <div style={{ background: "linear-gradient(180deg,#f6f8fb 0%, #ffffff 100%)", borderBottom: "1px solid #eef1f5" }}>
        <div style={{ maxWidth: MAXW, margin: "0 auto", padding: "44px 28px 36px" }}>
          ...
          <p style={{ fontSize: 16, color: SUB, lineHeight: 1.8, marginBottom: 24, maxWidth: 680 }}>
            All four TOEFL iBT sections — <strong style={{ color: INK }}>Reading, Listening, Speaking, and Writing</strong> — in the exact format of the real exam:
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
            <Link href="/toefl/test" style={{ display: "inline-block", background: GREEN, color: "#fff", borderRadius: 12, padding: "14px 28px", fontWeight: 800, fontSize: 16, textDecoration: "none", boxShadow: "0 10px 26px rgba(0,184,95,0.28)" }}>
              ▶ Start the practice test →
            </Link>
            <Link href="/toefl/sample" style={{ display: "inline-block", background: "#fff", color: INK, border: "1.5px solid #d8dee6", borderRadius: 12, padding: "14px 24px", fontWeight: 800, fontSize: 15, textDecoration: "none" }}>
              🎁 Try a free sample
            </Link>
          </div>
          <p style={{ fontSize: 13, color: SUB, margin: 0, lineHeight: 1.65 }}>
            The sample is <strong style={{ color: INK }}>free · no login required</strong> — try the real format and timer before you start. Full test: <strong style={{ color: INK }}>200 credits = 5 attempts</strong> · <strong style={{ color: INK }}>500 credits = unlimited</strong>
          </p>
        </div>
      </div>
        ...
        <div style={{ display: "grid", gap: 14, marginBottom: 44 }}>
          {SECTIONS.map((s) => (
            <div key={s.tag} style={{ display: "flex", alignItems: "center", gap: 18, borderRadius: 16, border: "1px solid #e8ecf1", background: "#fff", padding: "18px 20px", boxShadow: "0 1px 2px rgba(16,24,40,0.04)" }}>
              <span style={{ fontSize: 12, fontWeight: 900, color: "#fff", background: s.color, borderRadius: 8, padding: "6px 10px", flexShrink: 0 }}>{s.tag}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: INK }}>{s.title}</div>
                <div style={{ fontSize: 13.5, color: SUB, marginTop: 4, lineHeight: 1.6 }}>{s.desc}</div>
              </div>
            </div>
          ))}
        </div>

        <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 800, color: INK, marginBottom: 16 }}>Frequently asked questions</h2>
