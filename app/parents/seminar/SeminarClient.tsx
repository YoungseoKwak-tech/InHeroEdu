...
/**
 * /seminar — Free parent-facing seminar landing page.
 * Audience: Korean parents (학부모 그룹 / 카톡 / library). Styled in the /parents
 * community language (white, clean, green accent) for trust + conversion.
 *
 * Seminar: 2026-06-13 (Sat) 20:00 KST, online (Zoom), free, first-come 30 seats.
 * To wire the real signup, change APPLY_URL below to the registration link
 * (Google Form / KakaoTalk open chat / etc.).
 */
...
// === EDIT THIS to your real signup link (Google Form / 카톡 오픈채팅 / etc.) ===
const APPLY_URL = "#apply";
const SEATS = 30;
// Saturday 2026-06-13 20:00 KST
    ...
    <div style={{ background: "#f6f8fa", color: INK, fontFamily: "inherit" }}>
      ...
      <section style={{ background: DARK, color: "#fff", padding: "64px 20px 56px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <span
            style={{
              display: "inline-block", background: "rgba(0,184,95,0.18)", color: "#5fe0a0",
              border: "1px solid rgba(0,184,95,0.45)", borderRadius: 999, padding: "6px 14px",
          ...
          <p style={{ fontSize: 15.5, lineHeight: 1.8, color: "#9fb0c0", marginTop: 14 }}>
            경기외고 국제부에서 IBDP를 마치고, AP 13과목을 혼자 독학하고, AMC 최고점, 반도체 대기업 최연소 인턴,
            아마존 출판, TEDx 운영까지 — 이 모든 걸 <b style={{ color: "#dde7f0" }}>“누군가 그려준 지도” 없이</b> 직접 찾아냈습니다.
          ...
          <div
            style={{
              marginTop: 30, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)",
              borderRadius: 16, padding: "22px 22px", display: "grid", gap: 12,
            }}
          >
            ...
            <div style={{ marginTop: 8 }}><CTAButton block /></div>
          </div>

          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 26 }}>
            {CRED.map((c) => (
              <span key={c} style={{ fontSize: 12.5, color: "#aebccb", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 999, padding: "5px 11px" }}>
                {c}
              </span>
            ))}
          </div>
      ...
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "56px 20px 10px" }}>
        <h2 style={{ fontSize: 25, fontWeight: 800, letterSpacing: "-0.02em", margin: 0 }}>
          이번 세미나에서 다루는 것
        </h2>
        <div style={{ display: "grid", gap: 12, marginTop: 22 }}>
          {COVERS.map(([t, d], i) => (
            <div key={t} style={{ display: "flex", gap: 14, background: "#fff", border: "1px solid #e6ebf0", borderRadius: 14, padding: "18px 18px" }}>
              <div style={{ flexShrink: 0, width: 30, height: 30, borderRadius: 8, background: "rgba(0,184,95,0.12)", color: GREEN, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
                {i + 1}
              </div>
              <div>
      ...
      <section id="apply" style={{ maxWidth: 860, margin: "0 auto", padding: "52px 20px 80px", textAlign: "center" }}>
        ...
        <div style={{ marginTop: 22 }}><CTAButton /></div>
        <p style={{ fontSize: 12.5, color: "#90a0b0", marginTop: 16 }}>
