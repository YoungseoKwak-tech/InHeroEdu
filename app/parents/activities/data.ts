...
export const DEEP_GUIDES: DeepGuide[] = [
  ...
  },
  {
    id: "webapp",
    emoji: "💻",
    title: "웹·앱 프로젝트 — 0원으로 출시하기",
    tagline: "코딩으로 실제 문제를 푸는 플랫폼/도구를 직접 만들어 공개·운영하는 법",
    meta: [
      { label: "비용", value: "무료 (호스팅·DB 무료 티어)" },
      { label: "소요", value: "MVP 기준 4~10주" },
      { label: "난이도", value: "★★★ (코딩 학습 필요)" },
    ],
    overview:
      "직접 만든 웹/앱은 ‘문제를 발견하고 해결책을 코드로 구현했다’는 가장 강력한 이공계 스파이크입니다. 핵심은 화려한 기능이 아니라 ‘진짜 사용자에게 쓰인다’는 임팩트 — 라이브 URL과 사용자 수, GitHub 커밋 기록이 그대로 증빙이 됩니다.",
    phases: [
      { title: "1단계 · 문제 & 아이디어", steps: [
        "본인이나 주변의 ‘실제 불편’ 하나를 고른다 (예: 단어 암기, 봉사 매칭, 동아리 일정).",
        "MVP(최소기능) 한 가지로 좁힌다 — 기능 욕심이 가장 흔한 실패 원인.",
        "비슷한 서비스 3개를 직접 써 보고 ‘내 것만의 차별점’을 한 줄로 정한다.",
      ] },
      { title: "2단계 · 기술 학습 & 스택 선택", steps: [
        "무료 코스로 기초를 잡는다 (freeCodeCamp, The Odin Project, Harvard CS50).",
        "스택: 웹은 HTML/CSS/JS + React(Next.js), 앱은 React Native/Flutter. 백엔드·DB는 Supabase/Firebase 무료 티어.",
        "AI 코딩 도구(Cursor 등)로 가속하되, 코드를 이해하면서 쓴다(설명 못 하면 감점 요인).",
      ] },
      { title: "3단계 · 설계 & 개발", steps: [
        "화면 흐름(와이어프레임)을 Figma(무료)로 먼저 그린다.",
        "핵심 기능부터 구현해 ‘작동하는 데모’를 가장 먼저 만든다.",
        "Git/GitHub로 버전관리한다 — 꾸준한 커밋 기록 자체가 노력의 증빙.",
      ] },
      { title: "4단계 · 배포 & 운영", steps: [
        "Vercel/Netlify(무료)로 배포해 실제 접속 가능한 URL을 만든다.",
        "애널리틱스를 붙여 방문자·가입자 수를 추적한다.",
        "학교·커뮤니티에서 베타 사용자를 모집하고 피드백을 반영한다.",
      ] },
      { title: "5단계 · 성장 & 임팩트", steps: [
        "사용자/가입자 수, 실제 활용 사례를 기록한다(스크린샷·후기).",
        "GitHub에 오픈소스로 공개하고 README를 정리하면 협업·신뢰가 더해진다.",
        "해커톤·공모전에 출품해 외부 검증(수상)을 노린다.",
      ] },
      { title: "6단계 · 활동 기재", steps: [
        "Common App: ‘Founder/Developer, 「서비스명」 (URL)’ — Activity Type: Computer/Technology.",
        "사용자 수·다운로드·수상을 설명 칸에 숫자로 압축한다.",
      ] },
    ],
    links: [
      { label: "freeCodeCamp — 무료 코딩 학습", url: "https://www.freecodecamp.org" },
      { label: "The Odin Project — 웹개발 풀코스", url: "https://www.theodinproject.com" },
      { label: "CS50 — Harvard 컴퓨터과학 입문", url: "https://cs50.harvard.edu/x/" },
      { label: "Vercel — 무료 배포", url: "https://vercel.com" },
      { label: "Figma — 화면 설계(무료)", url: "https://www.figma.com" },
    ],
    pitfalls: [
      "기능 과욕으로 MVP를 못 내고 끝남 — 하나만 제대로.",
      "배포를 안 함 — 라이브 URL 없는 프로젝트는 설득력이 약하다.",
      "AI 복붙만 하고 코드를 이해 못 함 — 면접·에세이에서 드러난다.",
      "사용자 0명 — 임팩트(쓰는 사람)가 없으면 ‘만들기 연습’에 그친다.",
    ],
    commonApp:
      "‘앱을 만들었다’가 아니라 ‘어떤 문제를 누구를 위해 풀었고 몇 명이 썼나’를 적는다. 직책은 Founder/Developer, 단체명에 서비스명+URL, 설명에 사용자 수·핵심 기능·임팩트를 전보문체로.",
    storyTip:
      "만든 동기가 개인 경험에서 나오면 에세이와 직결된다(예: 동생의 학습 문제 → 학습 앱). ‘없어서 내가 만들었다’는 능동성이 핵심.",
  },
  {
    id: "nonprofit",
    emoji: "🤝",
    title: "비영리·캠페인 — 임팩트를 조직하기",
    tagline: "혼자 하는 봉사를 넘어, 팀·캠페인·(필요시) 비영리로 지속적 임팩트를 만드는 법",
    meta: [
      { label: "비용", value: "0~소액 (웹·인쇄·홍보)" },
      { label: "소요", value: "학기~연 단위 (지속성이 핵심)" },
      { label: "난이도", value: "★★☆ (리더십·꾸준함)" },
    ],
    overview:
      "입학사정관은 ‘봉사 시간 수’보다 ‘내가 주도해 만든 변화의 규모·지속성·리더십’을 봅니다. 일회성 봉사가 아니라, 문제를 정의하고 팀을 조직해 캠페인/이니셔티브로 키운 경험이 강력합니다.",
    phases: [
      { title: "1단계 · 문제 & 미션", steps: [
        "구체적인 사회 문제 하나(지역·환경·교육 격차 등)와 측정 가능한 목표를 정한다.",
        "‘왜 하필 내가 이 문제인가’ — 개인적 연결을 분명히 한다.",
      ] },
      { title: "2단계 · 팀 & 구조", steps: [
        "핵심 팀 2~5명을 모집하고 역할을 나눈다(리더십의 직접 증빙).",
        "정기 회의·역할표·기록 — 지속성의 기반을 만든다.",
      ] },
      { title: "3단계 · 실행 & 규모화", steps: [
        "작게 파일럿 → 결과 측정 → 확장한다.",
        "SNS·웹으로 알리고, 학교·지역단체·기존 NGO와 파트너십으로 신뢰·규모를 키운다.",
      ] },
      { title: "4단계 · (선택) 공식화", steps: [
        "미국 501(c)(3) 비영리는 설립 절차·비용·세무 부담이 크다 — 처음엔 학교 클럽이나 기존 단체 산하로 시작하는 게 현실적.",
        "필요하면 fiscal sponsor(후원 단체)를 통해 비영리 자격 없이도 기부금을 받을 수 있다.",
      ] },
      { title: "5단계 · 임팩트 측정", steps: [
        "모금액·수혜자 수·이벤트 횟수·참여자 수 등을 숫자로 기록한다.",
        "전후 비교(before/after)로 변화를 보여준다.",
      ] },
      { title: "6단계 · 활동 기재", steps: [
        "Common App: ‘Founder/President, 「이니셔티브명」’ — Activity Type: Community Service/Advocacy.",
        "임팩트 숫자와 지속 기간을 강조한다.",
      ] },
    ],
    links: [
      { label: "GoFundMe — 모금 캠페인", url: "https://www.gofundme.com" },
      { label: "Canva — 홍보물 제작(무료)", url: "https://www.canva.com" },
    ],
    pitfalls: [
      "일회성 이벤트로 끝남 — 지속성이 없으면 약하다.",
      "숫자 없는 ‘좋은 일’ — 임팩트는 측정돼야 설득된다.",
      "이름만 있는 단체 — 실제 활동 실체가 있어야 한다.",
      "과장 — 검증 가능한 사실만 적는다.",
    ],
    commonApp:
      "시간 채우기가 아니라 ‘내가 조직해 만든 변화의 규모’가 핵심. 직책은 Founder/President, 설명에 수혜자 수·모금액·지속 기간을 숫자로.",
    storyTip:
      "문제와 나의 연결이 분명할수록 강하다. ‘내가 본 불공정 → 그래서 시작한 일’의 서사로 에세이와 잇는다.",
  },
  {
    id: "internship",
    emoji: "🔬",
    title: "연구 인턴십 — 콜드메일로 따내기",
    tagline: "비싼 유료 프로그램 없이, 대학 연구실·기업에 직접 연락해 리서치 경험을 얻는 법",
    meta: [
      { label: "비용", value: "무료 (유료 프로그램 대안 제시)" },
      { label: "소요", value: "컨택 1~3개월 + 방학 활동" },
      { label: "난이도", value: "★★★ (거절을 견디는 끈기)" },
    ],
    overview:
      "수백~수천만 원짜리 유료 리서치 프로그램이 아니어도, 교수에게 직접 콜드메일을 보내 연구 보조로 들어갈 수 있습니다. 거절률이 높지만 한 곳만 성공하면 강력합니다 — 그리고 ‘거절을 이겨낸 능동성’ 자체가 좋은 에세이 소재가 됩니다.",
    phases: [
      { title: "1단계 · 분야 좁히기 & 교수 리스트업", steps: [
        "관심 키워드를 정하고, 근처 대학·온라인으로 교수 20~40명을 리스트업한다(학과 페이지·Google Scholar).",
        "각 교수의 최근 논문 제목을 읽고 ‘내가 흥미로운 이유’를 한 줄씩 메모한다.",
      ] },
      { title: "2단계 · 콜드메일 작성", steps: [
        "짧고 구체적으로: 내가 누구인지 + 그 교수의 ‘특정 논문’이 왜 끌렸는지 + 내가 기여할 수 있는 것 + 무급·유연함.",
        "간단한 이력서·성적·관련 스킬을 첨부하고, 제목은 명확하게 쓴다.",
        "복붙 티가 나면 즉시 무시당한다 — 교수마다 한 문장은 반드시 개인화한다.",
      ] },
      { title: "3단계 · 대량 발송 & 후속", steps: [
        "20~40곳에 개별화해 보낸다. 1주 후 정중한 follow-up을 1회 보낸다.",
        "무응답·거절이 기본값 — 1~2곳만 ‘예스’면 성공이다.",
      ] },
      { title: "4단계 · 무료·구조화된 대안", steps: [
        "무응답이면: 학교 교사와 독립연구, 공개 데이터셋으로 자기주도 연구 → JEI 투고로 연결.",
        "온라인 연구 멘토십·대학 여름 무료 프로그램(주립대 등)도 탐색한다.",
      ] },
      { title: "5단계 · 인턴십 수행", steps: [
        "약속한 시간·결과를 반드시 지킨다(신뢰가 추천서로 이어진다).",
        "작은 기여라도 기록한다 — 데이터·코드·논문 공저·발표로 산출물을 남긴다.",
      ] },
      { title: "6단계 · 활동 기재", steps: [
        "Common App: ‘Research Intern, 「연구실/기관」’ — Activity Type: Research.",
        "구체적 기여와 산출물(데이터·논문·발표)을 명시한다.",
      ] },
    ],
    links: [
      { label: "Google Scholar — 교수·논문 검색", url: "https://scholar.google.com" },
      { label: "JEI — 독립연구를 논문으로", url: "https://emerginginvestigators.org" },
    ],
    pitfalls: [
      "복붙 콜드메일 — 개인화 없으면 즉시 무시된다.",
      "한두 곳 보내고 포기 — 숫자(20~40곳)와 끈기가 핵심.",
      "능력 과장 — 솔직하게 ‘배우며 기여하겠다’가 더 통한다.",
      "약속 불이행 — 신뢰를 잃으면 추천서도 없다.",
    ],
    commonApp:
      "‘인턴 했다’가 아니라 ‘무엇을 기여하고 무엇을 배웠나’를 적는다. 산출물(데이터·논문·발표)을 강조하고, Activity Type은 Research.",
    storyTip:
      "왜 그 분야였는지가 곧 너의 스토리다. 수십 번의 거절 끝에 한 곳을 얻은 과정 자체가 회복력·능동성의 증거가 된다.",
  },
];
